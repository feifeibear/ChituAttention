...
from chitu.int8_flash_attention.configs import *

from chitu.int8_flash_attention.flash_atten_fp import attention
    ...
    if "triton" in provider:
        if "int8" in provider:
            if "full" in provider:
                ...
                q_scale = torch.randn(
                    (BATCH, H, N_CTX), dtype=dtype, device=device, requires_grad=False
                )
                k_scale = torch.randn(
                    (BATCH, H, N_CTX), dtype=dtype, device=device, requires_grad=False
                )
                v_scale = torch.randn(
                    (BATCH, H), dtype=dtype, device=device, requires_grad=False
                )
                sm_scale = 1.3
                fn = lambda: attention_full_int8(
                    q, k, v, q_scale, k_scale, v_scale, causal, sm_scale
                )
            else:
                q = torch.randint(
                    -128,
...
if __name__ == "__main__":
    ...
    acc_test(BATCH=1, H=8, N_CTX=1024 * 3, HEAD_DIM=64, causal=False)


@pytest.mark.parametrize("Z, H, N_CTX, HEAD_DIM", [(1, 2, 1024 * 3, 64)])
