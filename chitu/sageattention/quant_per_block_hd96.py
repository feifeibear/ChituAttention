...
def q_kernel_per_block_int8(
    X, X_int8, BLK: tl.constexpr, Scale, L, C: tl.constexpr, scale_stride
):
    ...
    x = tl.load(
        x_ptrs, mask=(offs_m[:, None] < L) & ((tl.arange(0, 128) < 96)[None, :])
    )
    x *= C**-0.5 * 1.44269504
    ...
    tl.store(
        x_int8_ptrs,
        x_int8,
        mask=(offs_m[:, None] < L) & ((tl.arange(0, 128) < 96)[None, :]),
    )
...
def per_block_int8_hd96(q, k, BLKQ=128, BLKK=64):
    ...
    if q.dim() == 3:
        q_scale = torch.empty(
            (q.shape[-3], (q.shape[-2] + BLKQ - 1) // BLKQ, 1),
            device=q.device,
            dtype=torch.float32,
        )
        k_scale = torch.empty(
            (k.shape[-3], (k.shape[-2] + BLKK - 1) // BLKK, 1),
            device=q.device,
            dtype=torch.float32,
        )
    elif q.dim() == 4:
        q_scale = torch.empty(
            (q.shape[-4], q.shape[-3], (q.shape[-2] + BLKQ - 1) // BLKQ, 1),
            device=q.device,
            dtype=torch.float32,
        )
        k_scale = torch.empty(
            (k.shape[-4], k.shape[-3], (k.shape[-2] + BLKK - 1) // BLKK, 1),
            device=q.device,
            dtype=torch.float32,
        )
    ...
    q_kernel_per_block_int8[grid](
        ...
        L,
        C,
        q_scale.stride(0) if q_scale.dim() == 3 else q_scale.stride(1),
    )
    ...
    k_kernel_per_block_int8[grid](
        ...
        L,
        C,
        k_scale.stride(0) if k_scale.dim() == 3 else k_scale.stride(1),
    )
