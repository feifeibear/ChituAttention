...
def per_block_int8(tensor, BLOCK_M=128):
    num_blocks = (tensor.shape[2] // BLOCK_M) + (
        1 if tensor.shape[2] % BLOCK_M != 0 else 0
    )
    processed_tensors = []
    scales = []
    for i in range(num_blocks):
        start_idx = i * BLOCK_M
        end_idx = min(start_idx + BLOCK_M, tensor.shape[2])
        block_tensor = tensor[:, :, start_idx:end_idx, :]
        max_over_h, _ = torch.max(torch.abs(block_tensor), dim=2, keepdim=True)
        max_over_hw, _ = torch.max(max_over_h, dim=3, keepdim=True)
        scale = max_over_hw.to(torch.float32) / 127
        scaled_tensor = (block_tensor / scale).round().to(torch.int8).to(torch.float32)
        processed_tensors.append(scaled_tensor)
        scales.append(scale)
    final_tensor = torch.cat(processed_tensors, dim=2)
    final_scales = torch.cat(scales, dim=2)
    return final_tensor, final_scales
